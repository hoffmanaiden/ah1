#define PI 3.1415926535897932384626433832795

varying vec2 vUv;



float random(vec2 st) {
  return fract(sin(dot(st.xy, vec2(12.9898, 78.233))) * 43758.5453123);
}

vec2 rotate(vec2 uv, float rotation, vec2 mid) {
  return vec2(
    cos(rotation) * (uv.x - mid.x) + sin(rotation) * (uv.y - mid.y) + mid.x,
    cos(rotation) * (uv.y - mid.y) - sin(rotation) * (uv.x - mid.x) + mid.y
  );
}

//	Classic Perlin 2D Noise 
//	by Stefan Gustavson
//
vec2 fade(vec2 t) {return t*t*t*(t*(t*6.0-15.0)+10.0);}

vec4 permute(vec4 x){return mod(((x*34.0)+1.0)*x, 289.0);}

float cnoise(vec2 P){
  vec4 Pi = floor(P.xyxy) + vec4(0.0, 0.0, 1.0, 1.0);
  vec4 Pf = fract(P.xyxy) - vec4(0.0, 0.0, 1.0, 1.0);
  Pi = mod(Pi, 289.0); // To avoid truncation effects in permutation
  vec4 ix = Pi.xzxz;
  vec4 iy = Pi.yyww;
  vec4 fx = Pf.xzxz;
  vec4 fy = Pf.yyww;
  vec4 i = permute(permute(ix) + iy);
  vec4 gx = 2.0 * fract(i * 0.0243902439) - 1.0; // 1/41 = 0.024...
  vec4 gy = abs(gx) - 0.5;
  vec4 tx = floor(gx + 0.5);
  gx = gx - tx;
  vec2 g00 = vec2(gx.x,gy.x);
  vec2 g10 = vec2(gx.y,gy.y);
  vec2 g01 = vec2(gx.z,gy.z);
  vec2 g11 = vec2(gx.w,gy.w);
  vec4 norm = 1.79284291400159 - 0.85373472095314 * 
    vec4(dot(g00, g00), dot(g01, g01), dot(g10, g10), dot(g11, g11));
  g00 *= norm.x;
  g01 *= norm.y;
  g10 *= norm.z;
  g11 *= norm.w;
  float n00 = dot(g00, vec2(fx.x, fy.x));
  float n10 = dot(g10, vec2(fx.y, fy.y));
  float n01 = dot(g01, vec2(fx.z, fy.z));
  float n11 = dot(g11, vec2(fx.w, fy.w));
  vec2 fade_xy = fade(Pf.xy);
  vec2 n_x = mix(vec2(n00, n01), vec2(n10, n11), fade_xy.x);
  float n_xy = mix(n_x.x, n_x.y, fade_xy.y);
  return 2.3 * n_xy;
}

void main() {

  // black to white
  float bw = vUv.x;

  // cool vaporwave
  vec4 vaporwave = vec4(vUv.x, vUv.y, 0.5, 1);

  // going down
  float goingdown = 1.0 - vUv.y;

  // multiply
  float multi = vUv.y * 10.0;

  // modulo
  float modulo = mod(vUv.y * 10.0, 1.0);

  // conditional
  // strength = strength < 0.5 ? 0.0 : 1.0;

  // step(), not conditional
  // float strength = mod(vUv.x * 10.0, 1.0);
  // strength = step(0.5, strength);

  // vertical stripes
  // float strength = mod(vUv.x * 10.0, 1.0);
  // strength = step(0.7, strength);

  // grid - combined x & y stripes
  float strength = step(0.8, mod(vUv.x * 10.0, 1.0));
  strength += step(0.8, mod(vUv.y * 10.0, 1.0));

  // dot grid
  // float strength = step(0.8, mod(vUv.x * 10.0, 1.0));
  // strength *= step(0.8, mod(vUv.y * 10.0, 1.0));

  // dash grid
  // float strength = step(0.4, mod(vUv.x * 10.0, 1.0));
  // strength *= step(0.8, mod(vUv.y * 10.0, 1.0));

  // corners pattern
  // float barX = step(0.4, mod(vUv.x * 10.0, 1.0));
  // barX *= step(0.8, mod(vUv.y * 10.0, 1.0));
  // float barY = step(0.4, mod(vUv.y * 10.0, 1.0));
  // barY *= step(0.8, mod(vUv.x * 10.0, 1.0));
  // float strength = barY + barX;

  // crosses pattern
  // float barX = step(0.4, mod(vUv.x * 10.0, 1.0));
  // barX *= step(0.8, mod(vUv.y * 10.0 + 0.2, 1.0));
  // float barY = step(0.4, mod(vUv.y * 10.0, 1.0));
  // barY *= step(0.8, mod(vUv.x * 10.0 + 0.2, 1.0));
  // float strength = barY + barX;

  // absolute value
  // float strength = abs(vUv.x - 0.5);

  // min
  // float strength = min(abs(vUv.x - 0.5), abs(vUv.y - 0.5));

  // max
  // float strength = max(abs(vUv.x - 0.5), abs(vUv.y - 0.5));

  // cool DMT effect
  // float strength = mod(abs(vUv.x - 0.5), abs(vUv.y - 0.5));

  // square with black middle
  // float strength = step(0.2, max(abs(vUv.x - 0.5), abs(vUv.y - 0.5)));

  // frame
  // float strength = step(0.4, max(abs(vUv.x - 0.5), abs(vUv.y - 0.5)));

  // stupid
  // float square1 = step(0.2, max(abs(vUv.x - 0.5), abs(vUv.y - 0.5)));
  // float square2 = 1.0 - step(0.25, max(abs(vUv.x - 0.5), abs(vUv.y - 0.5)));
  // float strength = square1 * square2;

  // cool gradient
  // float strength = floor(vUv.x * 10.0) / 10.0;

  // cool gradient grid
  // float strength = floor(vUv.x * 10.0) / 10.0;
  // strength *= floor(vUv.y * 10.0) / 10.0;

  // random noise (TV fuzz)
  // float strength = random(vUv);

  // minecraft random noise
  // float xval = floor(vUv.x * 10.0) / 10.0;
  // float yval = floor(vUv.y * 10.0) / 10.0;
  // float strength = random(vec2(xval, yval));

  // angled grid
  // vec2 gridUv = vec2(
  //   floor(vUv.x * 10.0) / 10.0,
  //   floor((vUv.y + vUv.x) * 10.0) / 10.0
  // );
  // float strength = random(gridUv);

  // length
  // float strength = length(vUv);

  // circle shadow
  // float strength = length(vUv - 0.5);
  // float strength = distance(vUv, vec2(0.4, 0.6));

  // spotlight
  //  float strength = 1.0 - distance(vUv, vec2(0.5));


  // sunny pinhole
  // float strength = 0.015 / distance(vUv, vec2(0.5));


  // stretched sunny pinhole
  // vec2 lightUv = vec2(vUv.x * 0.2 + 0.4, vUv.y);
  // float strength = 0.015 / distance(lightUv, vec2(0.5));


  // star in space
  // vec2 lightUvX = vec2(vUv.x * 0.2 + 0.4, vUv.y);
  // float lightX = 0.015 / distance(lightUvX, vec2(0.5));
  // vec2 lightUvY = vec2(vUv.y * 0.2 + 0.4, vUv.x);
  // float lightY = 0.015 / distance(lightUvY, vec2(0.5));
  // float strength = lightY * lightX;


  // rotate star in space
  // float pi = 3.1415926535897932384626433832795;
  // vec2 rotatedUv = rotate(vUv, PI * 0.25, vec2(0.5));

  // vec2 lightUvX = vec2(rotatedUv.x * 0.2 + 0.4, rotatedUv.y);
  // float lightX = 0.015 / distance(lightUvX, vec2(0.5));

  // vec2 lightUvY = vec2(rotatedUv.y * 0.2 + 0.4, rotatedUv.x);
  // float lightY = 0.015 / distance(lightUvY, vec2(0.5));

  // float strength = lightY * lightX;



  // circle
  // float strength = step(0.25, distance(vUv, vec2(0.5)));

  // blurry ring
  // float strength = abs(distance(vUv, vec2(0.5)) - 0.3);

  // black ring
  // float strength = step(0.05, abs(distance(vUv, vec2(0.5)) - 0.3));

  // white ring
  // float strength = 1.0 - step(0.01, abs(distance(vUv, vec2(0.5)) - 0.3));


  // Wavvy ring (white)
  // vec2 wavedUv = vec2(
  //   vUv.x,
  //   vUv.y + sin(vUv.x * 26.0) * 0.1
  // );
  // float strength = 1.0 - step(0.01, abs(distance(wavedUv, vec2(0.5)) - 0.3));


  // Wavvy abstract
  // vec2 wavedUv = vec2(
  //   vUv.x + sin(vUv.y * 26.0) * 0.1,
  //   vUv.y + sin(vUv.x * 26.0) * 0.1
  // );
  // float strength = 1.0 - step(0.01, abs(distance(wavedUv, vec2(0.5)) - 0.3));


  // Wavvyer abstract
  // vec2 wavedUv = vec2(
  //   vUv.x + sin(vUv.y * 80.0) * 0.1,
  //   vUv.y + sin(vUv.x * 80.0) * 0.1
  // );
  // float strength = 1.0 - step(0.01, abs(distance(wavedUv, vec2(0.5)) - 0.3));

  // arc-tangent angle
  // float angle = atan(vUv.x, vUv.y);
  // float strength = angle;

  // center offset arc-tangent angle
  // float angle = atan(vUv.x - 0.5, vUv.y -0.5);
  // float strength = angle;

  // arc-tan fan
  // float angle = atan(vUv.x - 0.5, vUv.y -0.5);
  // angle /= PI * 2.0;
  // angle += 0.5;
  // float strength = angle;

  // arc-tan multi-fan
  // float angle = atan(vUv.x - 0.5, vUv.y -0.5);
  // angle /= PI * 2.0;
  // angle += 0.5;
  // angle *= 20.0;
  // angle = mod(angle, 1.0);
  // float strength = angle;


  // more angles
  // float angle = atan(vUv.x - 0.5, vUv.y -0.5);
  // angle /= PI * 2.0;
  // angle += 0.5;
  // float strength = angle;

  // another fan
  // float angle = atan(vUv.x - 0.5, vUv.y -0.5);
  // angle /= PI * 2.0;
  // angle += 0.5;
  // float strength = sin(angle * 100.0);

  // fan circle combo
  // float angle = atan(vUv.x - 0.5, vUv.y -0.5);
  // angle /= PI * 2.0;
  // angle += 0.5;
  // float sinusoid = sin(angle * 100.0);
  // float radius = 0.3 + sinusoid * 0.025;
  // float strength = 1.0 - step(0.01, abs(distance(vUv, vec2(0.5)) - radius));


  // basic perlin noise
  // float strength = cnoise(vUv * 5.0);

  // black or white perlin 
  // float strength = step(0.0, cnoise(vUv * 10.0));

  // neon glow perline
  // float strength = 1.0 - abs(cnoise(vUv * 10.0));

  // psychadelic
  // float strength = step(0.9, sin(cnoise(vUv * 10.0) * 20.0));


  // Clamp the strength
  strength = clamp(strength, 0.0, 1.0);

  // Color!
  vec3 blackColor = vec3(0.0);
  vec3 uvColor = vec3(vUv, 0.5);
  vec3 mixedColor = mix(blackColor, uvColor, strength);

  gl_FragColor = vec4(mixedColor, 1);

}